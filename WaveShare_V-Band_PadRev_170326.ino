...
int8_t PadRev = 0; // 0 = kein paddle revers   1= revers
...
void Pause_w(){
  ...
  delay(time_w); 
   ...
   for(ii=0; ii<= zeit;ii++){  
      if ((digitalRead(PKT) == LOW)&& (PadRev ==0)) bitSet(Merker, PKT_m);
      if ((digitalRead(STR) == LOW)&& (PadRev ==0))  bitSet(Merker, STR_m); 
      if ((digitalRead(PKT) == LOW)&& (PadRev ==1)) bitSet(Merker, STR_m);
      if ((digitalRead(STR) == LOW)&& (PadRev ==1))   bitSet(Merker, PKT_m);
...
//---------------------------------
void Version(){
   Keyboard.print("  Waveshare Kezer 170326  ");
...
void setup() {
   ...
   */
   ...
   if ((PadRev == 0)&& (digitalRead(STR) == LOW)){ PadRev =1;
      tone(M_Ton,Hz);
      delay(100);
      noTone(M_Ton);// Zielboard
      do{   delay(100);	
     // Serial.println("Revers");  //########
      }while((digitalRead(PKT) == LOW)|| (digitalRead(STR) == LOW)); // beide Tasten lösen
    }
    if((PadRev == 0)&&(digitalRead(PKT) == LOW)){ PadRev =2;
      tone(M_Ton,Hz);
       delay(100);
       noTone(M_Ton);// Zielboard	
      do{   delay(100);	
     // Serial.println("Junker");  //########
      }while((digitalRead(PKT) == LOW)|| (digitalRead(STR) == LOW)); // beide Tasten lösen
    }
}
//###################################
...
void loop() {
  if (PadRev ==2){
    //nur Junkertaste
    if(digitalRead(PKT)==LOW){Keyboard.press(KEY_RIGHT_CTRL);tone(M_Ton,Hz);}// Zielboard }
